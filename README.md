...
Domain Layer와 UseCase를 통한 비즈니스 로직 격리: UI가 데이터를 직접 가공하지 않도록 GetMartComparisonUseCase 등 세분화된 UseCase를 구현했습니다. Data 모듈은 데이터를 가져오는 역할만, UseCase는 비교/정렬 등 비즈니스 로직만 처리하며, Feature(UI)는 결과값만 구독하도록 결합도를 낮췄습니다.
